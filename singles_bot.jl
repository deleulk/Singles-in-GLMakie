...
function draw_board_bot!(ax, state::GameState)
    n = state.n
    empty!(ax)
    ax.limits = (0, n, 0, n)
...
function bot_has_solved!(fig::Figure,ax::Axis,state::GameState)
    if is_valid(state::GameState)
        draw_board_bot!(ax, state)
        println("De bot heeft het spel opgelost. Vive la 180!")
    end
end
...
function add_solve_button!(fig, ax, state::GameState)
    knop_bot = Button(fig[1, 3][6, 1], label = "Bot oplossen")
        ...
        solve!(state)
        draw_board!(ax, state)
        sleep(1.80) # Vive la 180
        bot_has_solved!(fig, ax, state)
    end
end
